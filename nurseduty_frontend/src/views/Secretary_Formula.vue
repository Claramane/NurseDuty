<template>
    <div class="secretary-formula-schedule">
        <h1>恩主公麻醉護理秘書公式排班神器</h1>
        <table>
            <thead>
                <tr>
                    <th>組別</th>
...
export default {
    ...
    setup() {
        ...
        const nurses = computed(() => store.getters['staff/activeNurses'].filter(nurse => nurse.role === 'secretary'))
        const currentSchedule = computed(() => store.getters['schedule/getFormulaSchedule']('secretary') || { type: 'secretary', formula_data: [] })
            ...
            try {
                ...
                if (!isSaved.value) {
                    const formulaData = currentSchedule.value.formula_data.map((schedule, index) => ({
                        ...
                        nurses: nurses.value
                            .filter(nurse => nurse.group === index + 1 && nurse.active && nurse.role === 'secretary')
                            .map(nurse => nurse.id)
                    }))
